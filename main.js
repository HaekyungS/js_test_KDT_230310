...
// root.children[1].style.zIndex = "2"

// console.log(a_key[1][a_key_key[1]])
// subject 위치 확인

CreateDoc("div", root.children[1]);
root.children[1].children[1].style.cssText = "display:none; position : relative; z-index:2"
...
for (i = 0; i < a_key[1][a_key_key[1]].length; i++) {
  ...
  // FontStyle(root.children[1].children[i])
  // 폰트 나중에 넣어주기
  ...
  root.children[1].children[1].children[i].addEventListener("click", function () {
    ...
    for (i = 0; i < a_key_key[0].length; i++) {
      ...
      root.children[2].children[1].children[i].children[0].textContent = a_key_key[0][i]
      console.log(student)
    ...
    // 보완할 점 : 강사님 데이터에서 가져온 단어가 줄바꿈이 이뤄지지않고 영역을 넘어가버림.
...
// main 부분 생성 및 스타일 적용
CreateDoc("main", root);
CommonStyle(root.children[2], "100%", "85%", a_key[2][a_key_key[2][0]][3]);
DisplayFlex(root.children[2], "flex")
root.children[2].style.position = "relative"
root.children[2].style.zIndex = "1"

// main > div
CreateDoc("div", root.children[2])
CommonStyle(root.children[2].children[0], "100%", "30%", "#fff")
...
CommonStyle(root.children[2].children[0].children[1], "70%");
